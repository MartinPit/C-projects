...
{
    if (rows < MIN_SIZE || rows > MAX_SIZE || cols < MIN_SIZE ||
            cols > MAX_SIZE) {
        return -1;
    }
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < cols; j++) {
            ...
            if (is_number(*cell)) {
                if (check_around(i, j, rows, cols, board) != get_number(*cell)) {
                    return -1;
                }
                continue;
            }
    ...
    if (mines_amount == 0 || mines_amount > rows * cols - 4) {
        return -1;
    }
