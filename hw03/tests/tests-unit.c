...
TEST(filter_protocol_basic1)
{
    ...
    CHECK(packet_count(filtered) == 0U);
...
TEST(filter_protocol_basic2)
{
    ...
    CHECK(get_packet(filtered, 0)->packet_header->orig_len == 93U);
    CHECK(get_packet(filtered, 1)->packet_header->orig_len == 66U);
    CHECK(get_packet(filtered, 5)->packet_header->orig_len == 1514U);
    CHECK(get_packet(filtered, 9)->packet_header->orig_len == 1514U);
